...
1.  `K` 키를 눌러 '수동 생성(Manual Create) 모드'로 전환합니다. (뷰어에 초록색 테두리가 생깁니다)
...
6.  연결을 마치려면 `Esc` 키를 눌러 '선택(Select) 모드'로 돌아옵니다.
...
#### 주석 (Annotation) 관계: 태그 + 설명 텍스트

태그에 대한 추가적인 설명이나 관련 텍스트 조각을 연결합니다.

1.  뷰어에서 주체가 되는 태그 1개와, 설명에 해당하는 텍스트 조각(회색 점선)들을 `Ctrl (Cmd) + 클릭`으로 함께 선택합니다.
2.  `R` 키를 누릅니다.
3.  태그와 텍스트 조각들 사이에 주석 관계(회색 점선)가 생성됩니다.

#### 노트(Note) 관계: 태그 + 노트 태그

장비, 라인, 계기 태그에 특정 'Notes & Holds' 태그를 연결하여 부가 정보를 제공합니다.

1.  뷰어에서 주체가 되는 **Equipment**, **Line**, 또는 **Instrument** 태그 1개 이상과, 연결할 **Notes & Holds** 태그 1개를 `Ctrl (Cmd) + 클릭`으로 함께 선택합니다.
2.  `N` 키를 누릅니다.
3.  선택된 주체 태그들과 노트 태그 사이에 노트 관계(청록색 점선)가 생성됩니다.
