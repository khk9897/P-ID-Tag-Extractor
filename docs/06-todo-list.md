...
### **성능 최적화 계획 (Phase 16+에서 진행 예정)**

#### **🎯 Phase 16-A: Core Performance Optimization (PdfViewer.tsx 리팩토링 시 적용)**
- [ ] **좌표 변환 캐싱 시스템** (PdfViewer.tsx - 예상 70-80% 성능 개선)
  - scale 기반 transformCoordinates 캐싱
  - useMemo로 Map 기반 좌표 캐시 구현
- [ ] **SVG 렌더링 최적화** (PdfViewer.tsx - 3단계)
  - Level 1: 조건부 렌더링 (보이지 않는 태그 스킵)
  - Level 2: 가상화 렌더링 (뷰포트 외부 태그 제거)
  - Level 3: 이벤트 델리게이션 (개별 핸들러 → 부모 델리게이션)
- [ ] **TagHighlight 컴포넌트 최적화** (PdfViewer.tsx)
  - 조건부 렌더링 (선택/하이라이트된 경우만)
  - 간소화된 highlight 버전 적용
- [ ] **useMemo 의존성 최적화** (PdfViewer.tsx)
  - 과도한 의존성 배열 최소화
  - currentTags, tagsMap 의존성 정리

#### **🎯 Phase 16-B: Memory & Relationship Optimization**  
- ✅ **관계(Relationships) 렌더링 최적화** (완료 - 2024.09.04)
  - Connection/Installation만 활성화 (60% 성능 개선)
  - 불필요한 관계 타입 스킵 적용
- [ ] **메모리 관리 최적화**
  - 페이지 변경 시 캐시 정리 시스템
  - 불필요한 선택 상태 정리
...
#### **🎯 Phase 16-C: Advanced Performance (장기)**
- [ ] **React.memo 최적화** (모든 대형 컴포넌트)
- [ ] **가상화 렌더링** (TagsPanel.tsx - 대량 태그 리스트)
- [ ] **Canvas 기반 렌더링** (선택사항 - PDF 뷰어 대안)
...
#### **🎯 Phase 16-D: 성능 측정 및 검증**
- [ ] **성능 측정 시스템 구축**
  - 페이지 전환 시간 측정 (`console.time('pageTransition')`)
  - React DevTools Profiler 활용
  - Chrome DevTools Memory 모니터링
  - DOM 노드 수 추적
- [ ] **성능 목표 달성 검증**
  - 페이지 전환: 10초 → 1-2초 목표
  - 메모리 사용량: 50MB → 20MB 목표  
  - DOM 노드: 10,000개 → 3,000개 목표
